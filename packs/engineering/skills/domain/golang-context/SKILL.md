...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-context` skill takes precedence.
...
- → See the `jimmy-skills@golang-concurrency` skill for goroutine cancellation patterns using context
- → See the `jimmy-skills@golang-database` skill for context-aware database operations (QueryContext, ExecContext)
- → See the `jimmy-skills@golang-observability` skill for trace context propagation with OpenTelemetry
- → See the `jimmy-skills@golang-design-patterns` skill for timeout and resilience patterns

## Enforce with Linters

Many context pitfalls are caught automatically by linters: `govet`, `staticcheck`. → See the `jimmy-skills@golang-linter` skill for configuration and usage.

...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-naming` skill takes precedence.
...
Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `jimmy-skills@golang-linter` skill for configuration and usage.

## Cross-References

- → See `jimmy-skills@golang-code-style` skill for broader formatting and style decisions
- → See `jimmy-skills@golang-structs-interfaces` skill for interface naming depth and receiver design
- → See `jimmy-skills@golang-linter` skill for automated enforcement (revive, predeclared, misspell, errname)

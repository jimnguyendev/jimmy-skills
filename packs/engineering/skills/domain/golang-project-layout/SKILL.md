...
→ See `jimmy-skills@golang-design-patterns` skill for detailed architecture guides with file trees and code examples.
...
Packages MUST be lowercase, singular, and match their directory name. → See `jimmy-skills@golang-naming` skill for complete package naming conventions and examples.
...
- **.golangci.yml** — linter config. See the `jimmy-skills@golang-linter` skill for the recommended configuration
...
→ See `jimmy-skills@golang-cli` skill for CLI tool structure and Cobra/Viper patterns. → See `jimmy-skills@golang-linter` skill for golangci-lint configuration. → See `jimmy-skills@golang-continuous-integration` skill for CI/CD pipeline setup. → See `jimmy-skills@golang-design-patterns` skill for architectural patterns.

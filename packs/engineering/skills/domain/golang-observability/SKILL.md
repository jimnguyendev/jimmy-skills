...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-observability` skill takes precedence.
...
See `jimmy-skills@golang-error-handling` skill for the single handling rule. See `jimmy-skills@golang-troubleshooting` skill for using observability signals to diagnose production issues. See `jimmy-skills@golang-security` skill for protecting pprof endpoints and avoiding PII in logs. See `jimmy-skills@golang-context` skill for propagating trace context across service boundaries. See `promql-cli` skill for querying and exploring PromQL expressions against Prometheus from the CLI.

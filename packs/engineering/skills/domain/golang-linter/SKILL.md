...
For CI pipeline setup (GitHub Actions with `golangci-lint-action`), see the `jimmy-skills@golang-continuous-integration` skill.
...
- → See `jimmy-skills@golang-continuous-integration` skill for CI pipeline with golangci-lint-action
- → See `jimmy-skills@golang-code-style` skill for style rules that linters enforce
- → See `jimmy-skills@golang-security` skill for SAST tools beyond linting (gosec, govulncheck)

...
- → See `jimmy-skills@golang-performance` skill for general caching strategy and when to use in-memory cache vs Redis vs CDN
- → See `jimmy-skills@golang-observability` skill for Prometheus metrics integration and monitoring
- → See `jimmy-skills@golang-database` skill for database query patterns that pair with cache loaders

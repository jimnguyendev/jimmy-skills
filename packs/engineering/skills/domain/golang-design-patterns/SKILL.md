...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-design-patterns` skill takes precedence.

# Go Design Patterns & Idioms

Idiomatic Go patterns for production-ready code. For error handling details see the `jimmy-skills@golang-error-handling` skill; for context propagation see `jimmy-skills@golang-context` skill; for struct/interface design see `jimmy-skills@golang-structs-interfaces` skill.
...
12. **Use `strings.Builder`** for concatenation in loops → see `jimmy-skills@golang-code-style`
...
17. **Use `crypto/rand`** for keys/tokens — `math/rand` is predictable → see `jimmy-skills@golang-security`
...
→ See `jimmy-skills@golang-structs-interfaces` for the `var _ Interface = (*Type)(nil)` pattern.

## Error Flow Patterns

Error cases MUST be handled first with early return — keep the happy path at minimal indentation. → See `jimmy-skills@golang-code-style` for the full pattern and examples.
...
Retry logic MUST check `ctx.Err()` between attempts and use exponential/linear backoff via `select` on `ctx.Done()`. Long loops MUST check `ctx.Err()` periodically. → See `jimmy-skills@golang-context` skill.

## Database Patterns

→ See `jimmy-skills@golang-database` skill for sqlx/pgx, transactions, nullable columns, connection pools, repository interfaces, testing.
...
- **Respect 12-factor app** principles — → see `jimmy-skills@golang-project-layout`
...
- → See `jimmy-skills@golang-data-structures` skill for data structure selection, internals, and container/ packages
- → See `jimmy-skills@golang-error-handling` skill for error wrapping, sentinel errors, and the single handling rule
- → See `jimmy-skills@golang-structs-interfaces` skill for interface design and composition
- → See `jimmy-skills@golang-concurrency` skill for goroutine lifecycle and graceful shutdown
- → See `jimmy-skills@golang-context` skill for timeout and cancellation patterns
- → See `jimmy-skills@golang-project-layout` skill for architecture and directory structure

...
- → See `jimmy-skills@golang-performance` skill for optimization patterns after identifying bottlenecks
- → See `jimmy-skills@golang-observability` skill for metrics, alerting, and Grafana dashboards for Go runtime monitoring
- → See `promql-cli` skill for querying Prometheus metrics during production incident investigation
- → See `jimmy-skills@golang-concurrency`, `jimmy-skills@golang-safety`, `jimmy-skills@golang-error-handling` skills

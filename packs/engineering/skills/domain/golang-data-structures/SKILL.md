...
A slice is a 3-word header: pointer, length, capacity. Multiple slices can share a backing array (→ see `jimmy-skills@golang-safety` for aliasing traps and the header diagram).
...
Key functions: `Sort`/`SortFunc`, `BinarySearch`, `Contains`, `Compact`, `Grow`. For `Clone`, `Equal`, `DeleteFunc` → see `jimmy-skills@golang-safety` skill.
...
For `Clone`, `Equal`, sorted iteration → see `jimmy-skills@golang-safety` skill.
...
- → See `jimmy-skills@golang-performance` skill for struct field alignment, memory layout optimization, and cache locality
- → See `jimmy-skills@golang-safety` skill for nil map/slice pitfalls, append aliasing, defensive copying, `slices.Clone`/`Equal`
- → See `jimmy-skills@golang-concurrency` skill for channels, `sync.Map`, `sync.Pool`, and all sync primitives
- → See `jimmy-skills@golang-design-patterns` skill for `string` vs `[]byte` vs `[]rune`, iterators, streaming
- → See `jimmy-skills@golang-structs-interfaces` skill for struct composition, embedding, and generics vs `any`
- → See `jimmy-skills@golang-code-style` skill for slice/map initialization style

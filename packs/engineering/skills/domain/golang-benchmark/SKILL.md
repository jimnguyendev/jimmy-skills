...
- → See `jimmy-skills@golang-performance` skill for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `jimmy-skills@golang-troubleshooting` skill for pprof setup on running services (enable, secure, capture), Delve debugger, GODEBUG flags, root cause methodology
- → See `jimmy-skills@golang-observability` skill for everyday always-on monitoring, continuous profiling (Pyroscope), distributed tracing (OpenTelemetry)
- → See `jimmy-skills@golang-testing` skill for general testing practices

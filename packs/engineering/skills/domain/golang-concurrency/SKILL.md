...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-concurrency` skill takes precedence.
...
- -> See `jimmy-skills@golang-performance` skill for false sharing, cache-line padding, `sync.Pool` hot-path patterns
- -> See `jimmy-skills@golang-context` skill for cancellation propagation and timeout patterns
- -> See `jimmy-skills@golang-safety` skill for concurrent map access and race condition prevention
- -> See `jimmy-skills@golang-troubleshooting` skill for debugging goroutine leaks and deadlocks
- -> See `jimmy-skills@golang-design-patterns` skill for graceful shutdown patterns

...
1. **Profile before optimizing** — intuition about bottlenecks is wrong ~80% of the time. Use pprof to find actual hot spots (→ See `jimmy-skills@golang-troubleshooting` skill)
...
**When external:** optimize that component instead — query tuning, caching, connection pools, circuit breakers (→ See `jimmy-skills@golang-database` skill, [Caching Patterns](references/caching.md)).
...
2. **Write an atomic benchmark** — isolate one function per benchmark to avoid result contamination (→ See `jimmy-skills@golang-benchmark` skill)
...
| Lock contention | mutex/block profile hot | → See `jimmy-skills@golang-concurrency` skill |
| Slow queries | DB time dominates traces | → See `jimmy-skills@golang-database` skill |
...
Automate benchmark comparison in CI to catch regressions before they reach production. → See `jimmy-skills@golang-benchmark` skill for `benchdiff` and `cob` setup.

## Cross-References

- → See `jimmy-skills@golang-benchmark` skill for benchmarking methodology, `benchstat`, and `b.Loop()` (Go 1.24+)
- → See `jimmy-skills@golang-troubleshooting` skill for pprof workflow, escape analysis diagnostics, and performance debugging
- → See `jimmy-skills@golang-data-structures` skill for slice/map preallocation and `strings.Builder`
- → See `jimmy-skills@golang-concurrency` skill for worker pools, `sync.Pool` API, goroutine lifecycle, and lock contention
- → See `jimmy-skills@golang-safety` skill for defer in loops, slice backing array aliasing
- → See `jimmy-skills@golang-database` skill for connection pool tuning and batch processing
- → See `jimmy-skills@golang-observability` skill for continuous profiling in production

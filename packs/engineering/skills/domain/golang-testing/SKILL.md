...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-testing` skill takes precedence.
...
→ See `jimmy-skills@golang-benchmark` skill for advanced benchmarking: `b.Loop()` (Go 1.24+), `benchstat`, profiling from benchmarks, and CI regression detection.
...
Many test best practices are enforced automatically by linters: `thelper`, `paralleltest`, `testifylint`. See the `jimmy-skills@golang-linter` skill for configuration and usage.

## Cross-References

- -> See `jimmy-skills@golang-stretchr-testify` skill for detailed testify API (assert, require, mock, suite)
- -> See `jimmy-skills@golang-database` skill (testing.md) for database integration test patterns
- -> See `jimmy-skills@golang-concurrency` skill for goroutine leak detection with goleak
- -> See `jimmy-skills@golang-continuous-integration` skill for CI test configuration and GitHub Actions workflows
- -> See `jimmy-skills@golang-linter` skill for testifylint and paralleltest configuration

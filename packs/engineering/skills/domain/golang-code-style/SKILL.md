...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@golang-code-style` skill takes precedence.

# Go Code Style

Style rules that require human judgment — linters handle formatting, this skill handles clarity. For naming see `jimmy-skills@golang-naming` skill; for design patterns see `jimmy-skills@golang-design-patterns` skill; for struct/interface design see `jimmy-skills@golang-structs-interfaces` skill.
...
- Functions SHOULD have **≤4 parameters**. Beyond that, use an options struct (see `jimmy-skills@golang-design-patterns` skill).
...
Many rules are enforced automatically: `gofmt`, `gofumpt`, `goimports`, `gocritic`, `revive`, `wsl_v5`. → See the `jimmy-skills@golang-linter` skill.

## Cross-References

- → See the `jimmy-skills@golang-naming` skill for identifier naming conventions
- → See the `jimmy-skills@golang-structs-interfaces` skill for pointer vs value receivers, interface design
- → See the `jimmy-skills@golang-design-patterns` skill for functional options, builders, constructors
- → See the `jimmy-skills@golang-linter` skill for automated formatting enforcement

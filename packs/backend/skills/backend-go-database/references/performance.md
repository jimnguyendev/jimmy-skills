...
→ See `jimmy-skills@backend-go-observability` skill for database metrics and query monitoring. → See `promql-cli` skill for querying pool metrics (`db_open_connections`, `db_in_use_connections`, `db_idle_connections`) via CLI.

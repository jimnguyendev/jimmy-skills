...
- → See the `jimmy-skills@backend-go-concurrency` skill for goroutine cancellation patterns using context
- → See the `jimmy-skills@backend-go-database` skill for context-aware database operations (QueryContext, ExecContext)
- → See the `jimmy-skills@backend-go-observability` skill for trace context propagation with OpenTelemetry
- → See the `jimmy-skills@backend-go-design-patterns` skill for timeout and resilience patterns

## Enforce with Linters

Many context pitfalls are caught automatically by linters: `govet`, `staticcheck`. → See the `jimmy-skills@backend-go-linter` skill for configuration and usage.

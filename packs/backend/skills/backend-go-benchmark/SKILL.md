...
- → See `jimmy-skills@backend-go-performance` skill for optimization patterns to apply after measuring ("if X bottleneck, apply Y")
- → See `jimmy-skills@backend-go-troubleshooting` skill for pprof setup on running services (enable, secure, capture), Delve debugger, GODEBUG flags, root cause methodology
- → See `jimmy-skills@backend-go-observability` skill for everyday always-on monitoring, continuous profiling (Pyroscope), distributed tracing (OpenTelemetry)
- → See `jimmy-skills@backend-go-testing` skill for general testing practices

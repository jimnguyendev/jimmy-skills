...
Requires `import _ "net/http/pprof"` (see `jimmy-skills@backend-go-troubleshooting` skill for secure setup):
...
Short-lived allocations creating GC churn. Objects are allocated and freed rapidly — each one is cheap individually but the aggregate volume triggers frequent GC cycles. Common sources: `fmt.Errorf` in hot paths (allocates every call), interface boxing (`any` arguments), string-to-byte conversions, slice growth without preallocation. → See `jimmy-skills@backend-go-performance` skill for allocation reduction patterns.
...
Allocation rate is the bottleneck, not computation. The Go runtime is spending more time allocating and collecting garbage than running your code. Switch to the `alloc_objects` heap profile to find which functions allocate the most, then → See `jimmy-skills@backend-go-performance` skill for reduction patterns.

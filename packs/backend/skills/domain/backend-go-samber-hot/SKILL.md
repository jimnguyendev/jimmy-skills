...
- → See `jimmy-skills@backend-go-performance` skill for general caching strategy and when to use in-memory cache vs Redis vs CDN
- → See `jimmy-skills@backend-go-observability` skill for Prometheus metrics integration and monitoring
- → See `jimmy-skills@backend-go-database` skill for database query patterns that pair with cache loaders
- → See `promql-cli` skill for querying Prometheus cache metrics via CLI

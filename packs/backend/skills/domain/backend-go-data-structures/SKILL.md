---
name: backend-go-data-structures
...
A slice is a 3-word header: pointer, length, capacity. Multiple slices can share a backing array (→ see `jimmy-skills@backend-go-safety` for aliasing traps and the header diagram).
...
Key functions: `Sort`/`SortFunc`, `BinarySearch`, `Contains`, `Compact`, `Grow`. For `Clone`, `Equal`, `DeleteFunc` → see `jimmy-skills@backend-go-safety` skill.
...
For `Clone`, `Equal`, sorted iteration → see `jimmy-skills@backend-go-safety` skill.
...
- → See `jimmy-skills@backend-go-performance` skill for struct field alignment, memory layout optimization, and cache locality
- → See `jimmy-skills@backend-go-safety` skill for nil map/slice pitfalls, append aliasing, defensive copying, `slices.Clone`/`Equal`
- → See `jimmy-skills@backend-go-concurrency` skill for channels, `sync.Map`, `sync.Pool`, and all sync primitives
- → See `jimmy-skills@backend-go-design-patterns` skill for `string` vs `[]byte` vs `[]rune`, iterators, streaming
- → See `jimmy-skills@backend-go-structs-interfaces` skill for struct composition, embedding, and generics vs `any`
- → See `jimmy-skills@backend-go-code-style` skill for slice/map initialization style

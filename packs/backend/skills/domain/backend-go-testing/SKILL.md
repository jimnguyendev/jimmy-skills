...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@backend-go-testing` skill takes precedence.
...
→ See `jimmy-skills@backend-go-benchmark` skill for advanced benchmarking: `b.Loop()` (Go 1.24+), `benchstat`, profiling from benchmarks, and CI regression detection.
...
Many test best practices are enforced automatically by linters: `thelper`, `paralleltest`, `testifylint`. See the `jimmy-skills@backend-go-linter` skill for configuration and usage.

## Cross-References

- -> See `jimmy-skills@backend-go-stretchr-testify` skill for detailed testify API (assert, require, mock, suite)
- -> See `jimmy-skills@backend-go-database` skill (testing.md) for database integration test patterns
- -> See `jimmy-skills@backend-go-concurrency` skill for goroutine leak detection with goleak
- -> See `jimmy-skills@backend-go-continuous-integration` skill for CI test configuration and GitHub Actions workflows
- -> See `jimmy-skills@backend-go-linter` skill for testifylint and paralleltest configuration

---
name: backend-go-structs-interfaces
...
- → See `jimmy-skills@backend-go-naming` skill for interface naming conventions (Reader, Closer, Stringer)
- → See `jimmy-skills@backend-go-design-patterns` skill for functional options, constructors, and builder patterns
- → See `jimmy-skills@backend-go-code-style` skill for value vs pointer function parameters (distinct from receivers)

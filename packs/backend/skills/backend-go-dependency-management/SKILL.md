---
name: backend-go-dependency-management
...
The `jimmy-skills@backend-go-popular-libraries` skill contains a curated list of vetted, production-ready libraries. Prefer recommending packages from that list. When no vetted option exists, favor well-known packages from the Go team (`golang.org/x/...`) or established organizations over obscure alternatives.
...
- → See `jimmy-skills@backend-go-continuous-integration` skill for Dependabot/Renovate CI setup
- → See `jimmy-skills@backend-go-security` skill for vulnerability scanning with govulncheck
- → See `jimmy-skills@backend-go-popular-libraries` skill for vetted library recommendations

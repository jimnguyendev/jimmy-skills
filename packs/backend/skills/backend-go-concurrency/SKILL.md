...
> **Community default.** A company skill that explicitly supersedes `jimmy-skills@backend-go-concurrency` skill takes precedence.
...
- -> See `jimmy-skills@backend-go-performance` skill for false sharing, cache-line padding, `sync.Pool` hot-path patterns
- -> See `jimmy-skills@backend-go-context` skill for cancellation propagation and timeout patterns
- -> See `jimmy-skills@backend-go-safety` skill for concurrent map access and race condition prevention
- -> See `jimmy-skills@backend-go-troubleshooting` skill for debugging goroutine leaks and deadlocks
- -> See `jimmy-skills@backend-go-design-patterns` skill for graceful shutdown patterns

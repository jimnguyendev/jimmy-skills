...
**When external:** optimize that component instead — query tuning, caching, connection pools, circuit breakers (→ See `jimmy-skills@backend-go-database` skill, [Caching Patterns](references/caching.md)).
...
| Lock contention | mutex/block profile hot | [Lock-free patterns](references/lock-free-patterns.md), → See `jimmy-skills@backend-go-concurrency` skill |
| Slow queries | DB time dominates traces | → See `jimmy-skills@backend-go-database` skill |
...
- [Caching Patterns](references/caching.md) — algorithmic complexity, compiled patterns, singleflight, work avoidance, LFU vs LRU
- [Lock-Free Patterns](references/lock-free-patterns.md) — atomic ops, atomic.Value, channel async, lazy allocation, batch counters

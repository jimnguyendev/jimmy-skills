...
→ See `jimmy-skills@backend-go-concurrency` skill for `singleflight` API details and `sync.Map` vs `RWMutex` decision guidance. → See [Lock-Free Patterns](lock-free-patterns.md) for singleflight as part of a broader lock-free decision framework. → **Generics alternative:** Use `github.com/samber/go-singleflightx` to avoid interface{} boxing overhead; expect 2-4x faster result retrieval compared to the standard library's `singleflight.Group`.

### Local cache eviction: LFU vs LRU

Choosing the right eviction strategy affects hit rate significantly. Both implement a common `LocalCache` interface (`Get`, `Set`, `Delete`, `Clear`, `Close`, `Metrics`) and use `atomic.AddInt64` for lock-free hit/miss counting.

**LFU (Ristretto)** — `github.com/dgraph-io/ristretto`

- Admission policy based on access frequency (TinyLFU algorithm)
- Cost-based eviction — assign different costs per item for memory budget control
- Highly concurrent: uses ring buffers and sharding internally
- Default config: 10M numCounters, 1GB maxCost, 64 bufferItems
- Best for: frequency-skewed access patterns (most real workloads)

**LRU (golang-lru)** — `github.com/hashicorp/golang-lru/v2`

- Doubly-linked list + hash map, evicts least recently used
- Size-based only (ignores cost parameter)
- Generic: `lru.Cache[string, any]`
- Best for: temporal access patterns (recent items are hot)

**High-performance LRU alternative:** `github.com/elastic/go-freelru` — merges hashmap and ringbuffer into contiguous memory, ~37x faster than sharded implementations.

| Scenario | Recommendation |
|---|---|
| Unknown access pattern | LFU (Ristretto) — better default |
| Time-based access (recent = hot) | LRU |
| Frequency-based (popular items) | LFU |
| Memory-budget-sensitive | LFU (cost-based eviction) |
| Simple, predictable | LRU |

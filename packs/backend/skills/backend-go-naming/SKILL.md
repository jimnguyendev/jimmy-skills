...
Many naming convention issues are caught automatically by linters: `revive`, `predeclared`, `misspell`, `errname`. See `jimmy-skills@backend-go-linter` skill for configuration and usage.

## Cross-References

- → See `jimmy-skills@backend-go-code-style` skill for broader formatting and style decisions
- → See `jimmy-skills@backend-go-structs-interfaces` skill for interface naming depth and receiver design
- → See `jimmy-skills@backend-go-linter` skill for automated enforcement (revive, predeclared, misspell, errname)

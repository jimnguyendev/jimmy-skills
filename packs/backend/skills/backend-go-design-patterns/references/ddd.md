...
For large systems, each context can be its own Go module in a workspace (`go.work`). See the `jimmy-skills@backend-go-project-layout` skill for workspace setup.

## Wiring

Wire dependencies in `cmd/server/main.go` using manual constructor injection.
